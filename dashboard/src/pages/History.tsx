...
/**
 * Combine provider + category en libellé court pour le filtre Compte.
 * Doit rester aligné avec MainActivity.accountLabel() (Android) et
 * BalanceBar.accountLabel() côté dashboard.
 */
...
export default function History() {
  ...
  /**
   * Liste des comptes (provider, category) effectivement présents dans les
   * transactions chargées pour la période courante. Sert à construire
   * dynamiquement le select "Tous comptes" — on ne propose que les comptes
   * qui ont au moins une transaction visible, plutôt que tous les
   * opérateurs/catégories possibles.
   */
  const accountKeys = useMemo(() => {
    const set = new Set<string>();
    for (const r of data) {
      const cat = r.category || "MONEY";
      set.add(`${r.provider}|${cat}`);
    }
    return Array.from(set)
      .map((k) => {
        const [p, c] = k.split("|");
        return { provider: p, category: c, label: accountLabel(p, c) };
      })
  ...
  const filtered = useMemo(() => {
    ...
    let res = data.filter((r) => {
      ...
      if (category && (r.category || "MONEY") !== category) return false;
    ...
    return res;
  }, [data, provider, category, type, device, search, sortKey, sortDir]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        />
        <select
          value={provider && category ? `${provider}|${category}` : provider ? `${provider}|` : ""}
          onChange={(e) => {
